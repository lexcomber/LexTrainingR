...
```{r, echo=F}
df1 <- data.frame(type = c("character", "complex", "double", "expression", "integer", "list", "logical", "numeric", "single", "raw"), test = c("is.character", "is.complex", "is.double", "is.expression", "is.integer", "is.list", "is.logical", "is.numeric", "is.single", "is.raw"), conversion = c("as.character", "as.complex", "as.double", "as.expression", "as.integer", "as.list", "as.logical", "as.numeric", "as.single", "as.raw"))
knitr::kable(df1, caption = "Core data types, associated tests and conversions") 
```

In the same way it is possible to test and coerce data **classes**:
```{r, echo=F}
df1 <- data.frame(class = c("character", "logical", "numeric", "vector", "matrix", "data.frame", "array", "factor"), test = c("is.character", "is.logical", "is.numeric", "is.vector", "is.matrix", "is.data.frame", "is.array", "is.factor"), conversion = c("as.character", "as.logical", "as.numeric", "as.vector", "as.matrix", "as.data.frame", "as.array", "as.factor"))
knitr::kable(df1, caption = "Core data classes, associated tests and conversions") 
```
...
# Some more specific examples {#foo}
...
```{r, echo=F}
df1 <- data.frame(NonAttributed = c("SpatialPoints", "SpatialLines", "SpatialPoints", "SpatialPixels", "SpatialGrid"), Attributed = c("SpatialPointsDataFrame", "SpatialLinesDataFrame", "SpatialPolygonsDataFrame", "SpatialPixelsDataFrame", "SpatialGridDataFrame"), ArcGIS_version = c("Point shapefiles", "Line shapefiles", "Polygon shapefiles", "Raster", "Grid")) 
knitr::kable(df1, caption = "The `sp` spatial data classes") 
```
...
# Self-Test Questions Part 1 # {#foo}
...
```{r, echo=F}
df1 = data.frame(LogicalOperator = c("==", "!=", ">", "<", ">=", "<=", "!", "&", "|"), Description = c("Equal", "Not equal", "Greater than", "Less than", "Greater than or equal", "Less than or equal", "Not (goes in front of other expressions)", "And (combines expressions)", "OR (combines expressions)"))
knitr::kable(df1, caption = "Logical operators") 
```
...
Firstly,  download the R file `getdata.R` from the practical folder (also included as an appendix in this document), and place it in your working directory.  Secondly, make sure you have all of the 'helper' packages installed: 'GISTools`, `rjson`, `RCurl` and `XML` installed.  If you do not have these, start up R and enter:
```{r, echo=T, eval=F}
