...
- generate a corresponce matric and some classic measures of accuracy
- generate some surfaces

# Background 
Between 2008 and 2012, the late Prof Pete Fisher and I supervised a Lybian PhD student, Dr Abdulhakim Khmag who was undertaking research on *Fuzzy change detection*, using a case study in Lybia. He was awared his thesis in January 2013. For his research Abdulhakim generated a fuzzy land cover map of the area around Tripoli in Lybia containing 5 classes: 
*Bare ground*, *Grazing land* , *Urban*, *Vegetation* and *Woodland*. He undertook some field work in 2010 and collected data to validate the land cover map. He selected 210 locations using a stratified random sample. In this the study area was divided into blocks, from which an average of 10 locations per block was selected randomly. At each location, Abdulhakim recorded the proportions of the different land covor classes for the area covered by the 30m pixel. He was the first research to collect fuzzy ground data. The data for this study includes the values recored by Abdulhakim and the fuzzy class memberships fromt he land cover map. For most of this workshop we will work with the crisp, Boolean class dervied from the memberships.
...
This shows that the are a number of contiuous and categorical variables. The first few columns of data indicate the ID (`PointID`) and the easting and northing of the data point.
...
# Map the data
It is instrictive to map the data to see what we have and the `eat` and `north` variables in `data` can be used to create a `SpaatialPointsDataFrame` class of object. These are defined in the `sp` package which is loaded with the `GISTools` package.
...
data.spdf <- SpatialPointsDataFrame(coords = data[,2:3], data = data, proj4string = lyb.proj)
```
...
A shapefile of Lybia provides a bit of background and the location of Tripoli adding further context: 
```{r eval=T}
...
So first, poulate the correspondence matrix: 
...
Then use the marhgonal totals and disgonals to detemine user and producer accuracies. 
```{r eval=T}
# Users accuracy
...
Then caultae the overall accuracya nd include in the reduentnat element
```{r eval=T}
...
The table could be wrtiient to a fil if you want: 
```{r eval=F}
write.csv(tab, file = "Table1.csv")
```

# Correspondence, Probabilities and Regression
The *Overall*, *User* and *Producer* accuracies can be considered as probabiliites: the probability that any pixel is correctly classified (*Overall*), the probabiloity that any pixel of any *Observed* class in the field is correctly proedicted (*Producer*), and the probability that any *Predicted* class in the classified data is correctly predicted (*User*). These can also be estimated using ordinary logistic regressions. First, a logit function needs to be defined to transform any value, *q*:
...
For *User* and *Producer* accuracies we need a different model. It is useful to consider a specific example, in this case the class of Grazing Land. Form the table, we can see that it can be represented by binary vectors of 210 elements, with the Feild data (reference) a vector with 39 elements scored as 1, the classified data (predicted) having 43 elements scored as 1, with 23 data elements scored as 1 in common. 
...
where $P(x = 1)$ is the probability that the classified land-cover class is correctly pre- dicted by the reference data, $y_1$, $b_0$ is the intercept term and $b_1$ is the slope. 

For the example of grazing land the code to do this is as follows. Note that the foirst dstage is to create the binary vectors `fs.class` and `rs.class` for denote the feild survey and remote sensing classes: 
```{r eval=T}
...
These data can hten be used as inputs to the GLM below. Note that the terms / inputs oto the GLM are inverted in the code below:
...
# Summary
You have bene introduced to the data and the usual way of developing measures of accurcy for remote sensing products such as land cover. The statistical bases for the probibailistic accuracy measures that are comonly derived from the correspondence matrix have been shown. In the next section you will strat to develop spatialy distributed measures of accuracy.